Page({
  ...
  delt(e) {
    ...
    var xx = equipmentdelt.bussEquipDetailSet
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (arr[i].index == e.detail.value) {
        ...
        equipmentdelt.belongToArea = '',
          equipmentdelt.bussEquipDetailSet = []
        this.setData({
      ...
      if (this.data.duang == 'added') {
        ...
        for (var i = 0; i < arrr.length; i++) {
          ...
          if (arrr[i].index == this.data.equipmentdeltindex) {
            ...
            arrr[i].number= this.data.value3, //数量
            ...
            arrr[i].attachId= this.data.attachId //上传文件ID
            if (this.data.attachName==''){
              arrr[i].attachName = '' //文件名称
            }else{
              arrr[i].attachName = attachName //文件名称
            }
  ...
  load(id) {
    ...
    wx.request({
      url: this.data.baseUrl + 'terminal/bussEquipPlanDetailAppProject.do?',
      data: {
        tmessage: {
          "query": {
            "relateId": id
      ...
      success: (res) => {
        ...
        console.log(res)
        var arrr = res.data.info.result.bussEquipDetailSet
        var y = 0
        for (var i = 0; i < arrr.length; i++) {
          y = y + parseInt(arrr[i].number)
        }
